...
/**
 * Implements the 68000 {@code MULU.W <ea>,Dn} instruction helper.
 */
public final class Mulu {
    ...
    public static int execute(
        ...
        long sourceValue = Integer.toUnsignedLong(Size.WORD.mask(sourceReader.read()));
        long destinationValue = Integer.toUnsignedLong(Size.WORD.mask(destinationReader.read()));
        int result = (int) (sourceValue * destinationValue);
    ...
    private static void requireWordSize(Size size) {
        if (size != Size.WORD) {
            throw new IllegalArgumentException("MULU on the Macintosh Plus 68000 supports only WORD size");
        }
    }
}
