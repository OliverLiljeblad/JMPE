...
/**
 * Dispatch-layer executor for decoded {@code ANDI #imm,SR}.
 */
public final class AndiToSrOp implements Op {
    ...
    public int execute(M68kCpu cpu, DecodedInstruction decoded) {
        ...
        if (!cpu.statusRegister().isSupervisorSet()) {
            throw new PrivilegeViolation("ANDI to SR");
        }
    ...
    private static EffectiveAddress.Immediate validate(DecodedInstruction decoded) {
        if (decoded.opcode() != Opcode.ANDI_TO_SR) {
            throw new IllegalArgumentException("AndiToSrOp requires opcode ANDI_TO_SR but was " + decoded.opcode());
        }
        if (decoded.size() != Size.WORD) {
            throw new IllegalArgumentException("ANDI_TO_SR must be decoded as a WORD operation");
        }
        if (!(decoded.src() instanceof EffectiveAddress.Immediate immediate)) {
            throw new IllegalArgumentException(
                    "ANDI_TO_SR runtime currently supports immediate source only but was " + decoded.src()
            );
        }
        if (!(decoded.dst() instanceof EffectiveAddress.Sr)) {
            throw new IllegalArgumentException(
                    "ANDI_TO_SR requires SR as the destination but was " + decoded.dst()
            );
        }
        if (decoded.extension() != 0) {
            throw new IllegalArgumentException("ANDI_TO_SR must not carry an extension payload");
        }
        return immediate;
    }
}
